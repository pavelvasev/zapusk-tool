...
#  end
  ...
  def to_s
    "zapusk: zdb_dir=#{self.dir}, cmd=#{self.cmd}, name=#{self.name}, state_dir=#{self.state_dir}"
  end

  def ready?
    if comps.length == 0
      warning "ready?: empty components list! this is not error, just warning!"
      info    "ready?: zdb_dir=#{self.dir}"
    end
    raise "ready?: zdb dir file test failed, is not a directory! zfb_dir=#{dir}" if !File.directory?(dir)
